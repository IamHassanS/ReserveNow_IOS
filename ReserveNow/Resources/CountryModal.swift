...
class CountryModel{
    ...
    init(forDialCode d_code : String? = nil,withCountry c_code : String? =  nil,_ flagCountry: String? = nil){
        ...
        if result.count != 0{
            self.flag = ""
         //   self.dial_code = "+\(result[0]. ?? "")"
          //  self.flag = result[0].country_flag ?? "" //UIImage(named: "\(self.country_code.lowercased())") ?? UIImage(named: "us")!
        }else{
