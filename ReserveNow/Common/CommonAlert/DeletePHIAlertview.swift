...
//            commonAlertView.removeFromSuperview()
    ...
    func setupAlert(alert title:String,alertDescription message:String? = nil,okAction:String,cancelAction:String? = nil, userImage:String? = nil)
    {
        ...
        self.ThemeChange()
        alertView.message_titleLb.setFont(font: .medium(size: .BODY)) 
        alertView.messageDescriptionLbl.setFont(font: .medium(size: .BODY))
        alertView.userImageView.tintColor = .appHostThemeColor
...
//            alertView.PHI_DeleteYesBtn.addTarget(self, action: #selector(self.successBtnPressed(_:)), for: .touchUpInside)
            ...
            alertView.PHI_FinalMessage_OK.setTitleColor(.appHostThemeColor, for: .normal)
...
//            window?.subviews[alertView]
            let isdarkStyle = alertView.traitCollection.userInterfaceStyle == .dark
                alertView.centerView.backgroundColor = .systemBackground
                alertView.message_titleLb.textColor = .label
                alertView.deletePHI_popupTwoOption.backgroundColor = .appHostThemeColor
            alertView.deletePHI_popupOneOption.backgroundColor = .appHostThemeColor
                alertView.PHI_DeleteNoBtn.customColorsUpdate()
//            commonAlertView.removeFromSuperview()
        }
